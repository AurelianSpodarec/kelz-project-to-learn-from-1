...
const PoliciesProvider = ({ children, isSimulated }) => {
  const { addAlert } = React.useContext(AlertsContext)
  const { provider } = React.useContext(ProviderManageContext)
  ...
  const {
    ...
    query: {
      ...
      status: status.filter(x => !!x).length === 0 ? undefined : status,
